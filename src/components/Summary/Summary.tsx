    ...
    <div className="card">
      <div className="mainInfo">
        <div className="flex flex-col">
          <div className="flex justify-between">
            ...
            <div className="text-xl">${monthly ? plan.price : plan.price * 12}/{monthly ? 'mo' : 'yr'}</div>
          </div>
          <Divider></Divider>
            ...
            {addons.map((addon) => (
              <div 
                ...
                className="flex justify-between text-base py-1 text-slate-400">
                  <div>{addon.title}</div>
                  <div>+${monthly ? addon.price : addon.price * 12}/{monthly ? 'mo' : 'yr'}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Divider></Divider>
        <div className="flex justify-between text-base text-slate-400 py-3">
          ...
          <div>${monthly ? addonsPrice : addonsPrice * 12}/{monthly ? 'mo' : 'yr'}</div>
        </div>
      </div>
    </div>
  );
